...
### 2026-04-13: User directive — PR body link to GitHub App
**By:** Ahmed Sabbour (via Copilot)
**What:** PR body should link to the GitHub App that created it
**Why:** User request — captured for team memory

---

### 2026-04-13: Identity parallel bug — three fixes from live testing
**By:** Ahmed Sabbour (via Copilot)
**What:**
1. gh pr create with bot token needs --repo {owner}/{repo} — without it gh tries to fork
2. GitHub App manifest needs checks: read and actions: read permissions — bots cannot see CI results without them
3. Parallel token scoping: chained TOKEN assignments lose values in backgrounded subshells. Use newline-separated statements instead.
**Why:** Discovered during live E2E testing on sabbour/kickstart PR #140. Single-token spawn template is safe; multi-token batch operations are affected.

---

### 2025-07-16: `squad identity create` is team-aware
**By:** EECOM (Core Dev)

## Context

`squad identity create` with no flags previously just printed usage help. Users with a team roster in `.squad/team.md` had to manually run `--role` for each agent role, or `--all` (which creates all 8 roles even if the team only uses 4).

## Decision

When no flags are given, `squad identity create` now auto-detects roles from `.squad/team.md`:

1. Parses the `## Members` table to extract role column
2. Maps each role to its canonical slug via `resolveRoleSlug()`
3. Deduplicates (multiple agents may share a slug)
4. Creates apps only for the team's actual roles

Falls back to usage help when team.md is absent or has no members.

## Rationale

- Reduces friction — one command instead of N `--role` calls
- Only provisions what the team needs (vs `--all` creating unused apps)
- Zero breaking changes: `--role`, `--all`, `--simple` all still work

---

### 2025-07-16: Identity-Aware Agent Spawning
**Author:** EECOM (Core Dev)

## Context

The identity system (GitHub App–based bot authentication) is tested and working. To productize it, the coordinator's spawn templates need to automatically inject identity context into agent prompts.

## Decisions

1. **Identity block is conditional.** The GIT IDENTITY block is only included in spawn prompts when `.squad/identity/config.json` exists. If no identity is configured, the block is omitted entirely — agents use default git auth.

2. **PR bodies must link to the GitHub App.** When identity is active, every PR body must include `🤖 Created by [{app_slug}](https://github.com/apps/{app_slug})` so commits are visibly attributed to the bot.

3. **Token resolution uses the built SDK dist, not raw imports.** The spawn template references `packages/squad-sdk/dist/identity/tokens.js` (the compiled output), not TypeScript source. This avoids requiring `ts-node` or a build step at agent runtime.

4. **Graceful fallback — identity failure never blocks agent work.** If token resolution fails, the identity config is missing, or any step in the identity resolution chain errors, the agent falls back to default auth silently. No spawn is ever blocked because of identity.

5. **Role slug mapping follows `resolveRoleSlug()` semantics.** The coordinator maps agent roles to identity role slugs (lead, backend, frontend, tester) with fallback to `lead` when a role-specific app isn't registered.

## Affected Files

- `.github/agents/squad.agent.md` — full and lightweight spawn templates
- `.squad/templates/issue-lifecycle.md` — PR body template

---

### 2025-07-29: `squad identity fix` for installation ID recovery
**Author:** EECOM
**Status:** Implemented

## Context

When `squad identity create` succeeds but the installation polling times out (e.g., in WSL where DNS/fetch can be flaky), the app is saved with `installationId: 0`. Previously there was no easy way to fix this — users had to manually edit JSON files.

## Decision

Added `squad identity fix --role <role>` subcommand with two modes:

1. **Auto-detect** (default): Generates a JWT from the saved PEM, calls `GET /app/installations`, and writes the detected ID to both storage files.
2. **Manual override** (`--installation-id <id>`): Directly sets the value without API calls.

Also enhanced `squad identity status` to flag roles with `installationId: 0` and print actionable `squad identity fix` commands.

## Why not a `set` command?

`fix` is more discoverable — it signals "something is broken and this repairs it." A generic `set` command would need more guardrails (what if someone sets appId?) and opens a broader surface area. The `fix` command is scoped to the one thing that actually breaks in practice.

## Affected files

- `packages/squad-cli/src/cli/commands/identity.ts` — `runFix()`, enhanced `runStatus()`, updated create timeout message
- `test/identity/fix-installation.test.ts` — 7 new tests

---

### 2025-07-29: Integrate installation resolution into `squad identity create`
**By:** EECOM
**Status:** Implemented

## What

- Removed the standalone `squad identity fix` subcommand.
- `squad identity create` now handles the full lifecycle: app creation → browser install → poll until installed.
- `create` is idempotent: re-running on a role with an existing app skips creation and resolves the missing installationId.
- Polling has no hard timeout — the user controls when to stop via Ctrl+C.

## Why

Ahmed's feedback: "Why does it have to be a 'fix' command?" The user should run ONE command and get a fully working identity. Splitting the flow across `create` + `fix` added unnecessary cognitive overhead.

## Impact

- All agents: `squad identity fix` no longer exists. If scripts or docs reference it, update them.
- `squad identity status` now suggests re-running `create` instead of `fix` for broken installations.

---

### 2025-07-29: Identity documentation reflects shipped implementation
**By:** PAO (DevRel)
**Status:** Documented

## What

Updated `docs/proposals/agent-github-identity.md` from a forward-looking proposal into an implementation guide. The proposal was written pre-shipment; the actual implementation differs in several key ways. The doc now reflects reality.

## Changes Made

1. **Status:** "Proposal" → "✅ Implemented" (2025-07-29)
2. **Added Quick Start** — 3-step setup for new users
3. **Added Implementation Status table** — shows what shipped (Tier 2 ✅, Tier 1 ✅, Tier 3 design-only)
4. **Updated CLI commands section:**
   - `fix` command removed (was proposed but never shipped)
   - `create` is now idempotent — handles both app creation and installation ID re-detection
   - `update` is the new command for re-detection (simplified the two-command proposal into one)
5. **Expanded Copilot CLI Integration section** to include actual GIT IDENTITY block flow with Pre-Spawn Identity Resolution
6. **Added Testing section** pointing to `scripts/test-identity-e2e.mjs`
7. **Updated Token Lifecycle:** JWT exp is 9 minutes (not 10) — clock skew buffer for WSL
8. **Key facts included:**
   - Token resolution uses `node:crypto` RS256 — zero npm dependencies
   - Graceful fallback — identity is optional, never blocks agent work
   - PR bodies include GitHub App link for attribution

## Why

- **Proposals are templates, not archives.** Once a feature ships, the proposal becomes the primary user-facing documentation. It should reflect reality, not aspirations.
- **CLI design simplified from proposal.** The proposed `fix` command was merged into `create` (via idempotency) + `update`. Fewer commands = easier to learn.
- **Implementation includes details proposals skip.** The 9-minute JWT expiry, graceful fallback behavior, and GIT IDENTITY block injection are crucial for users to understand reliability.

## Affected Files

- `docs/proposals/agent-github-identity.md` (updated, ~150 lines changed/added)
- `.squad/agents/pao/history.md` (learnings appended)

## No Breaking Changes

This is documentation-only. The shipped identity system is unchanged.
